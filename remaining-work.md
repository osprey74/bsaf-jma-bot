...
- [ ] 竜巻・土砂・記録的短時間大雨パーサーを削除（フェーズBの移管稼働確認後）
- [ ] 旧様式気象警報（VPWW53/54）weather-warning / special-warning を段階削除 — 洪水対応（フェーズB）完了が前提。経過措置は2028年まで並行配信のため観察しつつ判断
- [ ] JMABot を純・地象/海象/火山Botとして `bot-definition.json` / README を更新
